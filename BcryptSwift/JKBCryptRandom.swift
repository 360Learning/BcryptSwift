...
//  JKBCrypt
//
//  Created by Jean Barloy on 5/02/2017.
//  Copyright (c) 2017 Jean Barloy. All rights reserved.
//
...
class JKBCryptRandom : NSObject {
     ...
     */
    class func generateNumberBetween(first: Int32, second: Int32) -> Int32 {
        var low : Int32
        var high : Int32
    ...
    /**
     ...
     :returns: [Int32]
     */
    class func generateNumberSequenceBetween(first: Int32, second: Int32, ofLength length: Int, withUniqueValues unique: Bool) -> [Int32] {
        if length < 1 {
            return [Int32]()
        }
        ...
        if unique {
            ...
            var loop : Int = 0
            while loop < length {
                let number = JKBCryptRandom.generateNumberBetween(first: first, second: second)
                
                // If the number is unique, add it to the sequence
                if !JKBCryptRandom.isNumber(number: number, inSequence: sequence, ofLength: loop) {
            ...
            for i in 0 ..< length {
                sequence[i] = JKBCryptRandom.generateNumberBetween(first: first, second: second)
            }
        }
    ...
    class func chooseNumberFromSequence(sequence: [Int32], ofLength length: Int) -> Int32? {
        ...
        let number = JKBCryptRandom.generateNumberBetween(first: 0, second: Int32(length - 1))
    ...
    class func generateRandomDataOfLength(length: Int) -> NSData {
        ...
        var sequence = JKBCryptRandom.generateNumberSequenceBetween(first: 0, second: 255, ofLength: length, withUniqueValues: false)
    ...
    class func generateRandomSignedDataOfLength(length: Int) -> NSData {
        ...
        var sequence = JKBCryptRandom.generateNumberSequenceBetween(first: -128, second: 127, ofLength: length, withUniqueValues: false)
    ...
    class func generateRandomStringOfLength(length: Int) -> String {
        ...
        for i in 0 ..< length {
            ...
            randomString.append(Character(nextCharacter))
        }
